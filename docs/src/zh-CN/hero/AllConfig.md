...
| `maintainabilityGuide` | 可维护性与扩展指南 |
| `developmentWorkflow` | 开发工作流 |
| `extensionArchitecture` | 扩展架构说明 |
| `heroExtension` | Hero 扩展手册 |
...
| `imageTypes` | 图像类型页面 |
| `basicHero` | 基础 Hero 矩阵 |
| `colorBackground` | 颜色背景示例 |
| `imageBackground` | 图片背景示例 |
| `videoBackground` | 视频背景示例 |
| `shaderBackground` | Shader 背景示例 |
| `particlesBackground` | 粒子背景示例 |
| `layersBackground` | 分层背景示例 |
| `buttonThemes` | 按钮与特性矩阵 |
| `featuresConfig` | 完整特性配置页 |
