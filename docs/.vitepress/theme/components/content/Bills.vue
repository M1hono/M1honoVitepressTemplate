<script setup lang="ts">
    ...
    /**
     * @file Bills.vue - A smart component for financial transactions, using the user-specified design.
     * @author M1sh
     * @version 7.0.0
     */
    ...
    onUpdated(() => {
        descriptionRowRefs.value.forEach((row) => {
            if (!row) return;
            const textEl = row.querySelector<HTMLElement>(".detail-text");
            if (textEl) {
                const lineHeight = parseFloat(getComputedStyle(textEl).lineHeight);
                if (textEl.scrollHeight > lineHeight + 1) {
